 ...
 */
MPU6050::MPU6050(uint8_t address,I2C &i2c):  _i2c (&i2c){
    devAddr = address;
...
uint8_t MPU6050::dmpInitialize() {
    ...
    printf("X gyro offset = %i \n",xgOffsetTC);
    printf("Y gyro offset = %i \n",ygOffsetTC);
    printf("Z gyro offset = %i \n",zgOffsetTC);
    ...
    printf("Writing DMP code to MPU memory banks (%i bytes)\n",MPU6050_DMP_CODE_SIZE);
     
    if (writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE)) {
        printf("Success! DMP code written and verified.\n");

        // write DMP configuration
        printf("Writing DMP configuration to MPU memory banks (%i bytes in config def)\n",MPU6050_DMP_CONFIG_SIZE);
        ...
        if (writeProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE)) {
            ...
            printf("ERROR! code 2 DMP configuration verification failed.\n");
            return 2; // configuration block loading failed
        }
    } else {
        printf("ERROR! code 1 DMP code verification failed.\n");
 ...
 */
int8_t MPU6050::readBit(uint8_t devAddr, uint8_t regAddr, uint8_t bitNum, uint8_t *data, uint16_t timeout) {
    uint8_t b;
 ...
 * @return Status of read operation (true = success)
 */
int8_t MPU6050::readBitW(uint8_t devAddr, uint8_t regAddr, uint8_t bitNum, uint16_t *data, uint16_t timeout) {
    uint16_t b = 0;
 ...
 * @return Status of read operation (true = success)
 */
int8_t MPU6050::readBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t *data, uint16_t timeout) {
    // 01101001 read byte
 ...
 */
int8_t MPU6050::readBitsW(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint16_t *data, uint16_t timeout) {
 ...
 * @return Status of read operation (true = success)
 */
int8_t MPU6050::readByte(uint8_t devAddr, uint8_t regAddr, uint8_t *data, uint16_t timeout) {
 ...
 * @return Status of read operation (true = success)
 */
int8_t MPU6050::readWord(uint8_t devAddr, uint8_t regAddr, uint16_t *data, uint16_t timeout) {
 ...
 */
int8_t MPU6050::readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *data, uint16_t timeout)
{
    char command[1];
    command[0] = regAddr;
    char *redData = (char*)malloc(length);
    _i2c->write(devAddr<<1, command, 1, true);
             //ThisThread::sleep_for(chrono::milliseconds(1)); 

    _i2c->read(devAddr<<1, redData, length);
...
int8_t MPU6050::readWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *data, uint16_t timeout)
{
    return 0;
}
 ...
 * @return Status of operation (true = success)
 */
bool MPU6050::writeBitW(uint8_t devAddr, uint8_t regAddr, uint8_t bitNum, uint16_t data) {
    uint16_t w;
 ...
 * @return Status of operation (true = success)
 */
bool MPU6050::writeBitsW(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint16_t data) {
    //              010 value to write
    // fedcba9876543210 bit numbers
    //    xxx           args: bitStart=12, length=3
...
bool MPU6050::writeWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *data)
{
    return true;
}
  
uint16_t MPU6050::readTimeout(void)
{
    return 0;
}